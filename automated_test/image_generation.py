...
class image_generation():
    ...
    def generate_image(self, game_data, talon, stage=0, background_color=20):
        print(f'Current stage er {stage} og game data er')
        card_img_aspect = 0.7
        card_img_size = (np.round(232 * card_img_aspect).astype(int),
                         np.round(360 * card_img_aspect).astype(int))
        x_margin = np.round(60 * card_img_aspect).astype(int)
        y_margin = np.round(92 * card_img_aspect).astype(int)
        ...
        width = np.round(
            7 * card_img_size[0] + 6 * x_margin + border_margin * 2).astype(int)
        height = np.round(3000 * card_img_aspect).astype(int)
        ...
        for row in range(19):
            ...
            for col in range(7):
                ...
                if cur_card != 0:
                    value = 1
                    if (cur_card is None and col != 0) or (cur_card != 'Blank' and cur_card.is_facedown):
                        # Resize the image using the aspect ratio further up
                        card_img = self.generate_card(None, card_img_size)
                        ...
                        # Paste the card on the image
                        generated_img.paste(card_img, card_pos, mask=card_img)
                    elif cur_card == 'Blank' or cur_card is None and col == 0:
                        # Resize the image using the aspect ratio further up
                        card_img = self.generate_card('Blank', card_img_size)
                        ...
                        # Paste the card on the image
                        generated_img.paste(card_img, card_pos, mask=card_img)
                    else:  # Check if 0, because it's a numpy zero-array
                        # Resize the image using the aspect ratio further up
                        card_img = self.generate_card(cur_card, card_img_size)
